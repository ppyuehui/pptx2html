...
try:
    ...
    class PPTXToHTMLConverter:
        def __init__(self):
            ...
            # 配置文件路径（Windows 标准：%APPDATA%\pptx2html\config.json）
            appdata = os.environ.get("APPDATA", os.path.expanduser("~"))
            self._config_dir = Path(appdata) / "pptx2html"
        ...
        def setup_ui(self):
            ...
            default_ffmpeg_dir = self._config.get("ffmpeg_dir", str(Path(__file__).parent / ".ffmpeg"))
        ...
        def browse_ffmpeg_dir(self):
            ...
            if directory:
                ...
                # 保存到配置文件
                self._config["ffmpeg_dir"] = directory
                self._save_config()
                # 重新检测
                threading.Thread(target=self.check_ffmpeg_status, daemon=True).start()
